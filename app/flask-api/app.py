...
@app.route('/health')
def health(): return jsonify({'status': 'healthy', 'service': 'flask-api'}), 200
...
def get_items():
    ...
    for item in items: item['created_at'] = item['created_at'].strftime('%Y-%m-%d %H:%M:%S')
...
def create_item():
    ...
    conn.commit(); item_id = cursor.lastrowid; cursor.close(); conn.close()
...
def delete_item(item_id):
    ...
    if affected == 0: return jsonify({'error': 'Not found'}), 404
